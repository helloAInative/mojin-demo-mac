...
- 阶段 4：WebSocket 行情推送、自选股交易时段调度、收盘复盘 / 周报生成（按需 API + 收盘 / 周末自动触发，按 `(kind, period_key)` 幂等）与 Swift 端复盘历史 / 手动生成（`ReportClient`）已完成；新闻 / 研报 / 板块数据接入（`/api/v1/news|reports|sector/{code}` + 每日收盘后增量刷新）已完成，研报评级自动出「机构看多 / 看空」信号并同步到 Swift；盯盘页已接入「资讯 · 研报 · 板块」面板
