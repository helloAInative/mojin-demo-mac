...
- A 股池智能推荐（服务端）：涨幅榜 → 技术指标 → 研报/新闻 → AI 精排四层漏斗，每日收盘自动生成 Top5 + T+5 胜率回测闭环（`/api/v1/picks`）；Swift 复盘页已接入推荐卡（AI 精排一键触发、点击加自选跳盯盘）；v2 多因子：板块动量 / 隔夜美股情绪 / 新闻关键词 / KDJ·BOLL·BIAS 扩充，综合分 60 门槛 + 标签级胜率回测
