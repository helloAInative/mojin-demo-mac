...
- AI 智能降级（ROI #7）：连续失败 / 限流 / 鉴权问题自动熔断，自动分析在熔断期直接走本地规则摘要（不白等超时），手动分析随时试探恢复
- 失效归因周报（ROI #9）：周报解释本周 level 预警的失败原因（穿越未确认 / 未到价 + 偏离度 / 窗口未满），被忽略的 AI 反馈计数
- 多标的组合视图（ROI #10）：自选页「持仓组合」卡——总市值 / 浮盈 / 当日盈亏聚合，持仓行带占比条与止损止盈标记，点击跳盯盘
- 周报导出（ROI #11）：复盘历史一键导出 .md / 复制全文，方便分享存档
- A 股池智能推荐（服务端）：涨幅榜 → 技术指标 → 研报/新闻 → AI 精排多层漏斗，工作日 14:45 生成“今日尾盘、目标 T+1 正收益” Top5（`/api/v1/picks`）；推荐卡明确标注今日尾盘 / 上一交易日推荐及短线 / 中线 / 做T（做T仅已有底仓）；T+1 为主回测和学习口径，T+5 保留为中线参考；样本≥30的标签胜率保守反馈到当日评分，客户端可查看调权证据
- 每日数据导出 + 回放（ROI #12）：收盘自动归档 `data/archives/`，`GET /export/day` 可取任意交易日；Swift 复盘页历史回放（归档分时上 K 线 + 当日信号）与一键导出 .json
