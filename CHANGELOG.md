...
- D/E 系小项四连发（§D.2 / D.3 / D.4 / E.3）：通知点击回写 userAction（审计闭环）；同价位连续 3 条无动作自动收紧 levelLightPct 30%（误报自调，落 alert 信号记录）；AI 页健康度行（今日成功率/耗时/失败原因 tooltip）；全局快捷键 ⌘1–6 切 tab / ⌘R 分析 / ⌘T 委托
- 智能止损止盈 M2（C.5 参数自适应）：近 60 日日均振幅中位数分低/中/高波动档 → k（止损 ATR 系数）2.5/2/1.5、m（止盈系数）3.5/3/2.5 自动切换——低波动收紧、高波动放宽防毛刺；建议卡档位徽标
- 决策辅助（§C.1 / C.4）：买单复制后自动弹 AI 反向校验（30 秒倒计时默认不动，撤单/不动一键）；持仓详情弹窗「AI 减仓建议」按钮（短 prompt 输出 0-100% 比例 + 理由，仅展示）
- 决策辅助（§C.2 / C.3）：回测命中点画到日 K（▲ 净盈红 / ✕ 净亏绿，hover 看进出价与净幅，日 K 头部回测点数徽标）；同价位预警 5 分钟 ≥3 次折叠（第 3 次带「刚才提示过」、第 4 次起静默）
- 走势图进化（§B 批次三，B.4 逐笔回放）：`GET /quote/{code}/ticks`（东财 push2delay details 经网关）+ Swift TickBucketer 分钟分桶 + TickChart 柱状图（净买红/净卖绿、x 对齐分时槽位），点击柱下钻该分钟逐笔明细与买卖统计
- 走势图进化（§B 批次二，B.5 板块联动）：`SectorAlignment` 弱势判定（板块涨/个股跌 → 橙色提示徽标；逆板块走强 / 偏离 ±2%）挂盯盘页对比区，AI prompt 注入板块联动证据
- 走势图进化（§B 批次二，B.2）：DayChart 日 K 多周期——蜡烛图（红阳实心/绿阴空心）+ MA5/10/20 三线 + 底部成交量条 + 昨收虚线与收盘标注 + hover 十字线 OHLC/MA 提示，盯盘页分时下方 190px 接线
- 走势图进化（§B 批次一）：B.1 分时图右侧 level 事件栏（hit 绿✓+leadSec / 失效红× / 等待灰…，按交易时段槽位竖列）；B.3 均价线（VWAP）黄细线画入分时图；B.6 自选页 4 列热力图（红涨绿跌梯度，点击跳盯盘）
