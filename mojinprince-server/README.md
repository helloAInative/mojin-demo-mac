...
- ✅ 测试全绿（2026-09-21 复跑 `cargo test --offline`）：单元 27 项 + 集成 44 项（行情 5 / AI 5+6 / 数据 3 / 复盘 9 / 报告调度 4 / 数据接入 9 / 智能推荐 3），另有 5 项真实外网用例默认 `#[ignore]`；3 项数据接入 live 用例已联网验证通过
...
四层漏斗：东财涨幅榜 Top100（push2delay，剔除 ST/退市/次新/一字板/超买）→ 技术指标量化打分（MACD 金叉 30 / RSI 健康区 15 / 放量 20 / 箱体强势 15 / 近 20 日高 10 / 均线多头 10）→ 研报评级上调 +25、下调 −20、新闻热度加分 → AI 精排（密钥只在请求内透传不落库，失败自动降级量化序）。工作日 15:30 后调度器自动生成纯量化版；T+1/T+5 收盘价自动回写 `meta.outcome` 形成命中率闭环。
...
每日数据归档（ROI #12）：`spawn_report_scheduler` 顺带把**最近已收盘交易日**（工作日 15:10 后为当日；盘中 / 周末 / 周一自动回补上一交易日）的自选股分时（minute_bar）、当日收盘快照（quote 最后一条）、当日信号、AI 用量汇总与持仓快照写成 `{数据库目录}/archives/{date}.json`——同一天文件已存在即跳过，长期资产按日沉淀。导出 HTTP 端点与回放 UI 待做。
