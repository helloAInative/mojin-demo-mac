...
- 🚧 阶段 4：WebSocket 行情推送 + 自选股交易时段调度 + 收盘复盘 / 周报生成已完成；**新闻 / 研报 / 板块数据接入（§F.3–F.4）已完成**；**A 股池尾盘智能推荐已完成**：14:45 产出、T+1 主回测与自动调权、短线 / 中线 / 做T 分类，Swift 复盘页已接入。
...
`GET /api/v1/picks?date=YYYY-MM-DD`（缺省最近一天）：当日 Top5 尾盘推荐 + 近 30 天 T+1 主回测，同时保留 T+5 中线参考。
...
工作日 14:45 后调度器自动生成尾盘纯量化版；错过收盘则标注为“下一交易日回踩”。T+1/T+5 收盘价渐进回写 `meta.outcome`，`stats.tags` 和自动调权以 **T+1 胜率**为主口径；单标签样本 ≥30 才介入，单标签 ±10 分、总调整 ±20 分，证据记录在 `meta.auto_weight`。交易时点、T+1 目标及短线 / 中线 / 做T 类型记录在 `meta.plan`；“做T”只会对已有底仓的标的出现。
